...
int Particletype::Sortdata() {
	...
	constantfunction=meanpotential-energypercell1;
	cout << "constantfunction= " << constantfunction << endl;
	for (int i=0; i<totallines-1; ++i){
...
int Particletype::Simulation(int times, int functionform){
	...
	double timereference=0.0000000001;
		...
		for (int i=0; i<times; ++i){
			for (int j=0; j<totallines-1; ++j){
				if (constantfunction>0){
					...
					matrixsimulation[i][info1[j][0]-1][info1[j][1]-1][info1[j][2]-1][0]=round(((particlespercell-info[j][0])/(exp(-deviation)))*exp(-((constantfunction)*(timereference)*(i))-(deviation)));

					matrixsimulation[i][info1[j][0]-1][info1[j][1]-1][info1[j][2]-1][2]=exp(-((constantfunction)*(timereference)*(i))-(deviation));
					matrixsimulation[i][info1[j][0]-1][info1[j][1]-1][info1[j][2]-1][3]=((energypercell-info[j][1])/(exp(-deviation)))*exp(-((constantfunction)*(timereference)*(i))-(deviation));
				}
				if (constantfunction<0){
					matrixsimulation[i][info1[j][0]-1][info1[j][1]-1][info1[j][2]-1][0]=round(((particlespercell-info[j][0])/(exp(-deviation)))*exp((constantfunction)*(timereference)*(i)-(deviation)));
					matrixsimulation[i][info1[j][0]-1][info1[j][1]-1][info1[j][2]-1][2]=exp((constantfunction)*(timereference)*(i)-(-deviation));
					matrixsimulation[i][info1[j][0]-1][info1[j][1]-1][info1[j][2]-1][3]=((energypercell1-info[j][1])/(exp(-deviation)))*exp((constantfunction)*(timereference)*(i)-(deviation));
				...
				restrictionenergysuma[i]=restrictionenergysuma[i]+matrixsimulation[i][info1[j][0]-1][info1[j][1]-1][info1[j][2]-1][3];
			}
			time=time+1.0;
		}
	}
	else if (functionform==1){ //Potencia
		double deviation1=-0.0000001;
		double lol=0.000000003;
		double timecount=0.00000001;
		double enigma=1/(pow(deviation1,-lol*constantfunction));
		...
		for (int i=0; i<times; ++i){
			for (int j=0; j<totallines-1; ++j){
				...
				if (constantfunction>0){
					...
					matrixsimulation[i][info1[j][0]-1][info1[j][1]-1][info1[j][2]-1][0]=round(((particlespercell-info[j][0])/(pow(-deviation1,-lol*constantfunction)))*pow((timecount*i)-deviation1,-lol*constantfunction));
					matrixsimulation[i][info1[j][0]-1][info1[j][1]-1][info1[j][2]-1][2]=pow((timecount*i)-deviation1,-lol*constantfunction);
					matrixsimulation[i][info1[j][0]-1][info1[j][1]-1][info1[j][2]-1][3]=((energypercell-info[j][1])/(pow(-deviation1,-lol*constantfunction)))*pow((timecount*i)-deviation1,-lol*constantfunction);
	...
	for (int i=0; i<times; ++i){
		for (int j=0; j<xdimension; ++j){
			for (int k=0; k<ydimension; ++k){
				for (int l=0; l<zdimension; ++l){
					cout << "matrixsimulation[" << i << "][" << j << "][" << k << "][" << l << "][" << 0 << "]= " << matrixsimulation[i][j][k][l][3] << endl; 
				}
			}
		}
	}
